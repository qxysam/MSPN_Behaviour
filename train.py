...
def main():
    ...
    with Engine(cfg, custom_parser=parser) as engine:
        ...
        model = MSPN(cfg, run_efficient=cfg.RUN_EFFICIENT)# 建模
        device = torch.device(cfg.MODEL.DEVICE) # cpu还是gpu
        model.to(device) # 我们用的是GPU

        num_gpu = len(engine.devices) # 1
        # default num_gpu: 8, adjust iter settings
        cfg.SOLVER.CHECKPOINT_PERIOD = \
            int(cfg.SOLVER.CHECKPOINT_PERIOD * 8 / num_gpu) # 153600
        cfg.SOLVER.MAX_ITER = int(cfg.SOLVER.MAX_ITER * 8 / num_gpu) # 6144000
        ...
        max_iter = len(data_loader) # 49152000
        checkpoint_period = cfg.SOLVER.CHECKPOINT_PERIOD
        # tb_writer就相当于一个日志，保存你要做图的所有信息。第二句就是在你的项目目录下建立一个文件夹log，存放画图用的文件。刚开始的时候是空的。
        ...
        for iteration, (images, valids, labels) in enumerate(
            ...
            # # len(labels): 32; # len(valids): 32; len(images): 32
