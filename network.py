...
class conv_bn_relu(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding, 
        ...
        self.relu = nn.ReLU(inplace=True) #inplace-选择是否进行覆盖运算 https://www.cnblogs.com/wanghui-garcia/p/10642665.html
...
class ResNet_downsample_module(nn.Module):
    ...
    def __init__(self, block, layers, has_skip=False, efficient=False,
            zero_init_residual=False): #layers = [3, 4, 6, 3] 每一个module里面生成几个子block，这里对应的block是Bottleneck
        ...
        for m in self.modules():
            # 使用isinstance来判断m属于什么类型 https://www.runoob.com/python/python-func-isinstance.html
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out',
                        nonlinearity='relu') # 用一个正态分布生成值，填充输入的张量或变量。结果张量中的值采样自均值为0，标准差为sqrt(2/((1 + a^2) * fan_in))的正态分布。 “fan_out”保留反向传播时的量级。 https://blog.csdn.net/ccbrid/article/details/89353817
            elif isinstance(m, nn.BatchNorm2d):
                nn.init.constant_(m.weight, 1) # m.weight = 1
                nn.init.constant_(m.bias, 0) # m.bias = 0
    ...
    def _make_layer(self, block, planes, blocks, stride=1, efficient=False):
        ...
        return nn.Sequential(*layers) # ×layers这里意思是接收任意layer，然后依次相连
...
class Upsample_unit(nn.Module): 
    ...
    def __init__(self, ind, in_planes, up_size, output_chl_num, output_shape,
        ...
        if self.gen_skip: #这里需要横向大小匹配，in_planes从上到小依次为：2048,1024，512和256
    ...
    def forward(self, x, up_x):
        out = self.u_skip(x) #  Conv2d(in_planes, 256, kernel_size=(1, 1), stride=(1, 1))
        ...
        cross_conv = None
        if self.ind == 3 and self.gen_cross_conv: #在最后的输出需要为17
...
class Upsample_module(nn.Module):
    ...
    def forward(self, x4, x3, x2, x1):
        ...
        skip1 = [skip1_4, skip1_3, skip1_2, skip1_1] #每一个阶段的最后一组进行横向连接
...
class MSPN(nn.Module):
    ...
    def __init__(self, cfg, run_efficient=False, **kwargs):
        ...
        self.top = ResNet_top()
        self.stage_num = cfg.MODEL.STAGE_NUM #2
        self.output_chl_num = cfg.DATASET.KEYPOINT.NUM #17
        self.output_shape = cfg.OUTPUT_SHAPE #(64, 48)
        self.upsample_chl_num = cfg.MODEL.UPSAMPLE_CHANNEL_NUM #256
        self.ohkm = cfg.LOSS.OHKM #True
        self.topk = cfg.LOSS.TOPK #8
        self.ctf = cfg.LOSS.COARSE_TO_FINE #True
        self.mspn_modules = list() 
        for i in range(self.stage_num):
            if i == 0:
                has_skip = False #The first stage
            else:
                has_skip = True #The second stage
            if i != self.stage_num - 1: # The first stage
                gen_skip = True
                gen_cross_conv = True
    ...
    def _calculate_loss(self, outputs, valids, labels):
        ...
        for i in range(self.stage_num):#0,1
            for j in range(4):
                ind = j
                if i == self.stage_num - 1 and self.ctf: # The second stage(i=1) 
                    ind += 1 
    ...
    def forward(self, imgs, valids=None, labels=None):
        ...
        outputs = list()
        for i in range(self.stage_num):
            res, skip1, skip2, x = eval('self.stage' + str(i))(x, skip1, skip2) #eval() 函数用来执行一个字符串表达式，并返回表达式的值。https://www.runoob.com/python/python-func-eval.html
            outputs.append(res)
        #print('The model Single Stage is:')
        #print(eval('self.stage' + str(i)))
...
#下面这些不需要管，正式训练的时候我们直接调数据里的相关值，下面的只是测试程序是否跑通，可以直接删掉
